...
如果相交，你的算法应该返回相交的那个节点，如果不相交，则返回 null。
...
这个题直接的想法是使用 HashSet，但需要额外的存储空间。如果不用 HashSet  ，只使用两个指针，该如何做？
...
题目说了，如果最后的元素不足 `k` 个，就保持不变。这就是base case。
...
```
以上代码很好解释把，**因为回文是对称的，所以正着读和倒着读应该是一样的，这一特点是解决回文串问题的关键**。
...
那么最简单的办法就是，把原链表反转存入一个新的链表，然后比较这两条链表是否相同。关于如何反转链表，请参考上面的反转链表部分。
...
```
这么做的核心逻辑是什么？**实际上是把链表放入一个栈，然后再拿出来，这时候元素顺序就是反的**，只不过我们利用的是递归函数的堆栈而已。
