...
public class LinkedListCommonOperations {
     ...
     * 2024-06-27 重新遇到改题，没看该思路，直接解决
     * 整个思想跟 mysql 的 lru 脏页的有记录被删除的时候，加入 clean 链表一样，
     * 或者JDK 的 AQS 中，封装当前线程的 Node 节点从 CLH 等待队列在调用 condition.await 方法后，
     * 从 clh 阻塞队列被删除，加入等待队列一样，就是节点在不同的队列间跳跃。
     * 所以，还是要多看源码，太有用了
     */
     ...
     * 
     * 下面进行 leetcode 第 19 题，删除单链表的倒数第 k 个元素。
