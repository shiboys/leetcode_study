...
这基本和前面说到了数组去重问题解法思路是完全一样的。直接上代码，参考removeElement()
...
到这里，原来修改数组的这些题目就已经差不多了。数组中另一大类快慢指针的题目就是「滑动窗口算法」。滑动窗口算法会有一个专门的章节来讲解
...
现在你应该感觉到回文串问题和左右指针肯定有密切的联系，比如让你判断一个字符串是不是回文串，我们就可以写出代码，参见 isPalindrome(String s)。
...
如果回文串的长度为奇数，则它有一个中心字符串；如果回文串的长度为偶数，则可以认为它有两个中心字符串。我们可以先实现一个从中间向两边获取回文的方法
```java
...
原理很简单，diff[i] +=3 意味着给 nums[i..] 的所有元素都加了 3，diff[j+1] -=3 又意味着对 nums[j+1..] 再减 3，综合起来就是对 `nums[i..j]` 中的所有元素都加 3 了(优点类似于 Kafka 索引的 相对 offset )。
...
相信我们大家已经联想到差分数组技巧了，**`trips[i]` 代表着一组空间操作，旅客的上下车就相当于数组区间的加减；只要结果数组中的元素都小于 `capacity` ，就说明可以不超载所有的旅客**。
...
常规的方式是把 `s` 按空格 `split` 成若干单词，然后 `reverse` 这些单词的顺序，最后再把这些单词 `join` 成句子。但是这种方式使用了额外的空间，并不是「原地反转」单词。
