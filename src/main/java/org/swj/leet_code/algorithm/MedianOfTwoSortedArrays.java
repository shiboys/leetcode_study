 ...
 *
 * <p>输入：nums1 = [1,2], nums2 = [3,4] 输出：2.50000 解释：合并数组 = [1,2,3,4] ，中位数 (2 + 3) / 2 = 2.5
 */
public class MedianOfTwoSortedArrays {
   ...
   * 3、剩下的 index1 和 index2 的判断条件是 index1+index2 > middle ，一旦超过了总长度的一半则立马跳出，是为了保证上一次的 index1+index2 == middle
   ...
   * 这道题我解了 2 个小时左右，主要浪费在之前没有考虑上一次遍历的位置，只是粗暴的记录了 index1 的上一个位置和 index2 之间谁大取谁，这个逻辑有问题的
   * 后来换成记录前一个被遍历的的索引和素组，本来我是打算用面向对象建立一个类来标识当前的正在遍历的 数组和索引，以及一个old 对象来记录上次遍历的数组和索引
   * 后来想到 力扣比较要求性能，我就没有用 oop，而是使用了 int 的数组指针和数组的对象引用，共 2 对4个对象，后来发现用了 上一个遍历的数组和索引之后，
