...
public class MaxSubArray {
     ...
     * 实际上是不行的，因为子数组一定是连续的，按照我们当前 dp 数组的定义，并不能保证 nums[0..i] 中的最大子数组与 nums[i+1]
     * 这个元素是相临的，
     ...
     * 我们依然使用数学归纳法来寻找状态转移关系：假设我们已经算出了 dp[i-1]，如果推导出 dp[i] 那？
     * 可以做到，dp[i] 有两种选择：要么与前面的相邻子数组连接，形成一个和更大的子数组；要么不与前面的子数组连接，自成一派，自己作为一个子数组。
     * 如何选择？既然要求「最大子数组和」，当然选择结果更大的那个了。
