...
**DFS 算法就是回溯算法**，BFS 的核心思想应该不难理解，就是把一些问题抽象成图，从一个点开始，向四周扩散。一般来说，我们写 BFS 算法都是用「队列」这种数据结构，每次讲一个节点周围的所有节点加入队列。
...
这点很重要你，这个形式在普通 BFS 问题中都很常见，但是在 Dijkastra 算法中，我们修改了这种代码模式，不再使用 size 遍历控制，而是使用优先级队里的优先级功能，寻找最短路径。
...
形象店说，DFS 是先，BFS 是面；DFS 是单打独斗，BFS 是集体行动。这个应该比较容易理解吧。

**2、既然 BFS 那么好，为啥 DFS 还要存在**？

BFS 可以找到最短距离，但是空间复杂度高，而 DFS 的空间复杂度较低。

还是拿刚才的二叉树问题举例，假设给你的这个二叉树是满二叉树，节点为 N，对于 DFS 算法来说，空间复杂度无非就是队规堆栈，嘴还情况下顶多就是树的高度，也就是 `O(logN)`。
...
穷举呗，再简单点，如果你只能转一下锁，有几种可能？总共有 4 个位置，每个位置可由向上转，也可以向下转，也就有 8 中可能对吧。
