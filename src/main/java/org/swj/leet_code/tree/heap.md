...
因为，二叉堆在逻辑上其实一种特殊的二叉树(完全二叉树)，只不过存储在数组里。一般的二叉树，我们操作的是节点的指针，而在数组里，我们把数组索引作为指针：
```java
// 父节点的指针
int parent(int root) {
    return root / 2;
...
```
空出来的 4 个方法是二叉堆和优先级队列的奥妙所在，下面用图文来逐个解释
...
优先级队列是基于二叉堆实现的，主要操作是插入和删除。插入是先插到最后，然后上浮到正确位置；删除是调换位置后再删除，然后下沉到正确位置。核心代码也就 10 行。
