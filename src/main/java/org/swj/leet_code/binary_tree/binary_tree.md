...
1、你理解的二叉树前后后序遍历是什么，仅仅是三个顺序不同的 List 吗？
2、请分析，后续遍历有什么特殊之处？
3、情分析，为什么对多叉树没有中序遍历？
...
先不管所谓前后后序，单看 `traverse` 函数，你说它在做什么？

其实它就是一个能够遍历所有二叉树节点的函数，和你比那里数组或者链表本质上没有任何区别：

单链表和数组的遍历可以是迭代的，也可以是递归的，**二叉树这种结构无非就是二叉链表**，由于没有办法简单改写成迭代形式，所以一般来说二叉树的遍历框架都是递归的形式。

我们也注意到，只要是递归的形式遍历，都可以有前序位置和后序位置，分别在地柜之前和递归之后。
...
**这个问题的根本却别在于：一个节点在第几层，你从根节点遍历过来的过程就能顺带记录，用递归函数的参数就能传递下去；而以一个节点为根的正棵子树有多少节点，你需要遍历完子树后才能数清除，然后通过递归函数的返回值拿到答案**。
